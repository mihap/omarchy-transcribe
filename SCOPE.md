...
| R8 | Files restart prompt. Mike, live from a local add: closed and reopened Files with Ctrl-Q several times, no Transcribe entry; `nautilus -q` fixed it. Closing Nautilus windows does not quit the process, it stays up as a background D-Bus service, so the extension dir is never re-read. `plugin/setup` now ends with "Restart Files now? [Y/n]" (default yes, plain `read`, only on a tty and only when Nautilus is running) and runs `nautilus -q`. Non-tty prints the hint as before. Replaces the R1 "tell, do not quit" decision. README, QA.md updated. Version 0.5.1. | DONE | see git log |
