...
| R3 | No prompts: `omarchy-transcribe-install` downloads `DEFAULT_MODEL` when no model exists and installs `ggml-vulkan` whenever `omarchy-hw-vulkan` is true; the command's first-run path downloads the default model instead of asking. gum no longer used, dropped from `depends`; the R2 sourcing lines removed with it. | DONE | |
