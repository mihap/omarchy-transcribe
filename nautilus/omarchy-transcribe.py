...
class TranscribeAction(GObject.GObject, Nautilus.MenuProvider):
    def _launch_transcribe(self, paths):
        wrapper = shutil.which("omarchy-launch-floating-terminal-with-presentation")
        binary = shutil.which("omarchy-transcribe")
    ...
    def _make_item(self, paths):
        label = "Transcribe" if len(paths) == 1 else f"Transcribe {len(paths)} items"
        item = Nautilus.MenuItem(
            name="OmarchyTranscribeNautilus::transcribe",
            label=label,
            icon="media-record",
        )
