...
- [ ] Super+Space → Setup → Plugins → Remove Plugin → pick Transcribe → floating terminal asks to delete (git repo) → Yes. Then a **second** floating terminal opens: "Transcribe plugin removed. Cleaning up what it installed.", uninstalls the recorded packages (sudo prompt), deletes models and settings, "Cleanup finished.", notification "Transcribe removed".
